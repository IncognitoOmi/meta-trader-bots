...
IS_EVALUATION = True          # Set to False to skip straight to Master
EVAL_PROFIT_TARGET = 0.10     # 10% Profit Target
EVAL_DAILY_DD = 0.03          # 3% Daily Drawdown
EVAL_TRAILING_DD = 0.06       # 6% Trailing Drawdown

# 🚨 MASTER (FUNDED) PROP FIRM RULES 🚨a
MASTER_PAYOUT_TARGET = 27500
MASTER_PAYOUT_AMOUNT = 250
MASTER_MIN_PAYOUT_DAYS = 10
MASTER_DAILY_DD_LIMIT = 750         # 3% of 25k
MASTER_INITIAL_OVERALL_FLOOR = 24000 # 4% Max Drawdown
MASTER_TRAPDOOR_FLOOR = 25000.0      # Floor after 1st payout

# STRATEGY RISK RULES (PRE-PAYOUT)
# INITIAL_MAX_RISK = 1500.0  
# INITIAL_MAX_FLOATING_LOSS = -200.0    # 🚨 FLOATING LOSS KILL SWITCH 🚨

# # ⚙️ STRATEGY RISK RULES (POST-1ST PAYOUT)
# POST_PAYOUT_MAX_RISK = 250.0          
# POST_PAYOUT_MAX_FLOATING_LOSS = -225 

# STRATEGY RISK RULES (PRE-PAYOUT / EVALUATION PHASE)
INITIAL_MAX_RISK = 200.0  
INITIAL_MAX_FLOATING_LOSS = -100.0    # 🏆 The winning Kill Switch from your backtest

# ⚙️ STRATEGY RISK RULES (POST-1ST PAYOUT / MASTER PHASE)
# Bumped this up from 250 to 750 so you can actually hit subsequent payout targets
POST_PAYOUT_MAX_RISK = 200.0          
POST_PAYOUT_MAX_FLOATING_LOSS = -100.0
...
for i in range(1, len(df)):
    ...
    if active_trade:
        ...
        if trade_closed:
            ...
            if is_eval_active:
                # Update High Watermark for Trailing DD
                if running_capital > high_watermark:
                    high_watermark = running_capital
                    eval_trailing_floor = max(eval_trailing_floor, high_watermark - (STARTING_CAPITAL * EVAL_TRAILING_DD))
                
                # Check Blow conditions
...
print("="*160)
print(f"📊 PROP FIRM SIMULATION SUMMARY")
print("="*160)
