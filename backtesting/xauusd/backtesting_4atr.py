...
if not mt5.initialize(path="C:/Program Files/MetaTrader 5 - FP_master/terminal64.exe") or not mt5.login(login=account_login, password=account_password, server=broker_server):
    ...
    quit()
# print(f"✅ Bot ACTIVE MASTER| Single Pair: {symbol} | 2-Layer Sniper Mode 🦅\n")
...
# 🕒 TIMEZONE FIX: Funding Pips uses EET (Eastern European Time), NOT UTC!
# Hum pehle usko EET me localize karenge, fir exact IST me convert karenge.
...
for i in range(1, len(df)):
    ...
    # 🕒 Ab time strictly IST me chal raha hai without any manual +5:30
