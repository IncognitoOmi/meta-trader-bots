...
# 7-Layer Risk Multiplier (56)
def get_dynamic_lot(fixed_atr):
    calculated_lot = MAX_RISK / (56 * fixed_atr * 100.0)
...
print("\n" + "="*135)
print(f"{'ENTRY TIME (IST)':<20} | {'TYPE':<4} | {'RSI':<5} | {'ATR':<4} | {'ENTRY':<8} | {'EXIT TIME (IST)':<20} | {'EXIT':<8} | {'LOT':<5} | {'LAYERS':<6} | {'RESULT':<6} | {'PNL'}")
print("="*135)
...
for i in range(1, len(df)):
    ...
    curr_ist_time = curr['time'] + timedelta(hours=5, minutes=30)
    time_hm = curr_ist_time.hour * 100 + curr_ist_time.minute
    
    if active_trade:
        if direction == "BUY":
            # 7 Layers max
            while orders_open < 7 and curr['low'] <= layers_prices[orders_open - 1]:
                orders_open += 1
            ...
            if curr['low'] <= sl:
                ...
                total_pnl += pnl
                print(f"{str(entry_time):<20} | BUY  | {entry_rsi:<5.1f} | {entry_atr:<4.2f} | {entry_price:<8.2f} | {str(curr_ist_time):<20} | {sl:<8.2f} | {current_lot:<5.2f} | {orders_open - 1:<6} | LOSS   | ${pnl:.2f}")
                active_trade = False
                continue
                ...
                total_pnl += pnl
                print(f"{str(entry_time):<20} | BUY  | {entry_rsi:<5.1f} | {entry_atr:<4.2f} | {entry_price:<8.2f} | {str(curr_ist_time):<20} | {tp:<8.2f} | {current_lot:<5.2f} | {orders_open - 1:<6} | WIN    | +${pnl:.2f}")
                active_trade = False
                
        elif direction == "SELL":
            # 7 Layers max
            while orders_open < 7 and curr['high'] >= layers_prices[orders_open - 1]:
                orders_open += 1
            ...
            if curr['high'] >= sl:
                ...
                total_pnl += pnl
                print(f"{str(entry_time):<20} | SELL | {entry_rsi:<5.1f} | {entry_atr:<4.2f} | {entry_price:<8.2f} | {str(curr_ist_time):<20} | {sl:<8.2f} | {current_lot:<5.2f} | {orders_open - 1:<6} | LOSS   | ${pnl:.2f}")
                active_trade = False
                continue
                ...
                total_pnl += pnl
                print(f"{str(entry_time):<20} | SELL | {entry_rsi:<5.1f} | {entry_atr:<4.2f} | {entry_price:<8.2f} | {str(curr_ist_time):<20} | {tp:<8.2f} | {current_lot:<5.2f} | {orders_open - 1:<6} | WIN    | +${pnl:.2f}")
                active_trade = False
        ...
        if time_hm >= 1130 and not (1700 <= time_hm <= 1800):
            if close > ema and rsi <= 31 and has_lower_wick:
                ...
                # SL updated to 14 ATR to accommodate 7 layers (0 to 6)
                sl = entry_price - (14 * entry_atr)
                tp = entry_price + (2 * entry_atr)
                
                # Create 6 extra layers
                layers_prices = [entry_price - (i * 2 * entry_atr) for i in range(1, 7)]
                orders_open = 1
...
print("="*135)
print(f"📊 63-DAY SUMMARY | 7-Layer Grid Mode | Dynamic Lot (56 Mult) | Filters Active")
print("="*135)
