...
if not mt5.initialize(path="C:/Program Files/MetaTrader 5 - FP2/terminal64.exe") or not mt5.login(login=account_login, password=account_password, server=broker_server):
    ...
    quit()
print(f"✅ Bot ACTIVE | Indices & Commodities Matrix Engine (Detailed Logging) 🚀\n")

# ==========================================
# 🧠 2. INDICES & COMMODITIES CONFIG
# ==========================================
PAIR_SETTINGS = {
    "NDX100": {"min_atr": 5.0,  "atr_offset": 1.0,  "contract": 1.0,   "decimals": 2},
    "DJI30":  {"min_atr": 10.0, "atr_offset": 2.0,  "contract": 1.0,   "decimals": 2},
    "USOIL":  {"min_atr": 0.20, "atr_offset": 0.05, "contract": 100.0, "decimals": 3},
}
...
# ==========================================
# 🧠 3. CORE SIMULATION FUNCTION (With Prints)
# ==========================================
def simulate_strategy(df, symbol, settings, grid_layers):
    ...
    def get_fixed_atr(raw_atr):
        ...
        return max(settings["min_atr"], round(adj, settings["decimals"]))

    def get_dynamic_lot(fixed_atr):
        calculated_lot = MAX_RISK / (RISK_MULT * fixed_atr * settings["contract"])
        return max(0.01, min(math.floor(calculated_lot * 100) / 100.0, 50.0)) 
    ...
    print(f"{'ENTRY TIME (IST)':<19} | {'TYPE':<4} | {'RSI':<4} | {'ATR':<7} | {'ENTRY':<10} | {'EXIT TIME (IST)':<19} | {'EXIT':<10} | {'LOT':<4} | {'LAYERS':<6} | {'RESULT':<6} | {'TRADE PNL'}")
    print("-" * 150)
        ...
        if active_trade:
            ...
            if direction == "BUY":
                ...
                if curr['low'] <= sl:
                    ...
                    total_pnl += pnl
                    print(f"{entry_time_str:<19} | BUY  | {entry_rsi:<4.1f} | {entry_atr:<7.3f} | {entry_price:<10.3f} | {exit_time_str:<19} | {sl:<10.3f} | {current_lot:<4.2f} | {orders_open - 1:<6} | LOSS   | -${abs(pnl):.2f}")
                    active_trade = False
                    continue
                    ...
                    total_pnl += pnl
                    print(f"{entry_time_str:<19} | BUY  | {entry_rsi:<4.1f} | {entry_atr:<7.3f} | {entry_price:<10.3f} | {exit_time_str:<19} | {tp:<10.3f} | {current_lot:<4.2f} | {orders_open - 1:<6} | WIN    | +${pnl:.2f}")
                    active_trade = False
                ...
                if curr['high'] >= sl:
                    ...
                    total_pnl += pnl
                    print(f"{entry_time_str:<19} | SELL | {entry_rsi:<4.1f} | {entry_atr:<7.3f} | {entry_price:<10.3f} | {exit_time_str:<19} | {sl:<10.3f} | {current_lot:<4.2f} | {orders_open - 1:<6} | LOSS   | -${abs(pnl):.2f}")
                    active_trade = False
                    continue
                    ...
                    total_pnl += pnl
                    print(f"{entry_time_str:<19} | SELL | {entry_rsi:<4.1f} | {entry_atr:<7.3f} | {entry_price:<10.3f} | {exit_time_str:<19} | {tp:<10.3f} | {current_lot:<4.2f} | {orders_open - 1:<6} | WIN    | +${pnl:.2f}")
                    active_trade = False
...
# ==========================================
print("\n" + "="*95)
print(f"{'🏆 INDICES MULTI-LAYER PNL MATRIX (Net PnL in USD) | Capital: $'+str(STARTING_CAPITAL):^95}")
print("="*95)
