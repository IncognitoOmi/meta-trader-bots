...
# 🚨 TARGET RISK SETTINGS ($200 Max Loss)
MAX_RISK = 200.0  
MAX_FLOATING_LOSS = -200.0 
MIN_SAFE_ATR = 1.5  
...
if not mt5.initialize(path="C:/Program Files/MetaTrader 5 - FundingPips/terminal64.exe") or not mt5.login(login=account_login, password=account_password, server=broker_server):
    ...
    quit()
print(f"✅ Bot ACTIVE PH2| Single Pair: {symbol} | 2-Layer Grid Mode | Max Risk: ${MAX_RISK} 🦅\n")
...
def floor_atr(raw_atr):
    ...
    # 🚨 RULE 2: Low ATR Fix applied here
...
def get_dynamic_lot(raw_atr):
    ...
    # 🚨 2-LAYER FIXED MATH: Entry(4 ATR SL) + L1(2 ATR SL) = 6 Total ATR Multiplier
...
def emergency_close_all():
    print(f"\n🚨 KILL SWITCH TRIGGERED! Closing all live trades and pending limits for {symbol}...")
...
def execute_grid(signal_type, raw_atr, current_lot):
    ...
    # 🚨 FIXED SL: Directly at 4 ATR distance (for 2-Layer grid)
...
try:
    while True:
        ...
        # 🚨 Live Clock for precise IST matching
        ...
        # 🚨 HARD KILL SWITCH CHECK ($200 Limit)
        if global_pos:
        ...
        if df is not None and len(df) > 2:
            ...
            # Clean useless pending orders (agar entry win hoke close ho gayi toh pending hata do)
            ...
            if num_pos_actual > 0:
                ...
                # 🚨 TP LOGIC: Agar dusri entry trigger ho gayi hai, TP original entry pe laao
            ...
            # Entry Logic
                ...
                # 🚨 TIME FILTERS (Start >= 11:30 AM AND NOT between 17:00 and 18:00)
...
# MAX_RISK = 200.0  
# MAX_FLOATING_LOSS = -200.0 
# MIN_SAFE_ATR = 1.5  # Smart ATR Trap fix

# if not mt5.initialize(path="C:/Program Files/MetaTrader 5 - FundingPips/terminal64.exe") or not mt5.login(login=account_login, password=account_password, server=broker_server):
#     print("❌ MT5 Connection Fail!")
#     quit()
# print(f"✅ Bot ACTIVE | Single Pair: {symbol} | 3-Layer Sniper Mode | Max Risk: ${MAX_RISK} 🦅\n")
